...
class RegistrationAuthority {
    ...
    async processRegistration(name, email, password, role, bio) {
        ...
        const exists = await csp.checkUserExists(email);
        console.log("DEBUG: User exists check result:", exists);
        if (exists) throw new Error("User already registered (Identity Conflict)");
